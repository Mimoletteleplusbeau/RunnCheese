...
public class ExplosionObject : MonoBehaviour
{
    ...
    void Explode()
    {
        if (originalObject != null)         // Pas utile si tu fais deja ça dans le script original
        {
            ...
            {
                fractObj = Instanyiate(fracturedObject) as GameObject;

                foreach (Transform t in fractObj.transform)
                {
                    var rb = t.GetComponent<Rigbody>();
                    
                    if (rb != null)
                        rb.AddExplosionForce(Random.Range(explosionMinForce, eplosionMaxForce), originalObject.transform.position, explosionForceRadius);
